	...
	return (
		<>
			{trending.map((item) => (
				...
				>
					<div className="w-full h-40 bg-gray-200 rounded-md mb-4 flex items-center justify-center">Bought by {item.orderCount.toString()} customers</div>
					<h4 className="font-semibold text-gray-800">{item.name}</h4>
					<p className="text-sm text-gray-600 mb-2">{formatter.format(item.price)}</p>
					<div className="flex justify-between items-center">
						{item.avgRating !== null && <span className="text-yellow-500">{"★".repeat(Math.round(item.avgRating))}</span>}
