...
import {
  ...
  validateRevalidate,
  validateTags,
} from 'next/dist/server/lib/patch-fetch'
...
function replacer(key: any, value: any) {
  if (typeof value === 'bigint') {
      return { _bint_: value.toString() };
  }
  return value;
}

function reviver(key: any, value: any) {
  if (value && typeof value === 'object' && value._bint_) {
      return BigInt(value._bint_);
  }
  return value;
}
...
export function unstable_cache<T extends Callback>(
  ...
  const cachedCb = async (...args: any[]) => {
    ...
    const invocationKey = `${fixedKey}-${JSON.stringify(args, replacer)}`
    const cacheKey = await incrementalCache.fetchCacheKey(invocationKey)
    // $urlWithPath,$sortedQueryStringKeys,$hashOfEveryThingElse
    const fetchUrl = `unstable_cache ${pathname}${
      sortedSearch.length ? '?' : ''
    }${sortedSearch} ${cb.name ? ` ${cb.name}` : cacheKey}`
          ...
          if (cacheEntry.value.kind !== 'FETCH') {
            ...
            const cachedResponse =
              cacheEntry.value.data.body !== undefined
                ? JSON.parse(cacheEntry.value.data.body, reviver)
                : undefined
